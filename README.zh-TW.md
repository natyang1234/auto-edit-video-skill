...
全文上下文校準會把帶編號的整份逐字稿與相鄰句一起提供給每個待檢查 target，再用第二次本機複核檢查候選。只有精確、最小、可定位時間範圍且雙重高信心的 ASR 正字會自動套用；改數字、改標點、整句順稿、一般 word choice、未列入術語表的英文變更或不確定項目一律拒絕或留待確認。`working/transcript_semantic_review.json` 會記錄已檢查／總段數、已套用、待確認、拒絕與模型錯誤；未完整覆蓋時會停止後續剪輯規劃，逐字稿不離開 loopback。

經確認規則與英文術語校準後，系統會先統一正體，再做全文上下文校準，最後才寫入 SRT、GUI 字幕、重點字、字卡及精華標題。`zh-TW`、`zh-en` 與自動辨識到的中文都使用技能內附的 OpenCC 台灣詞彙字典，因此上下文 patch 以台灣繁體文字為準；只有明確選擇 `zh-CN` 才保留簡體。英文、數字、標點與逐字時間碼不會被改動，轉換紀錄保存在 `working/transcript_orthography.json`。
